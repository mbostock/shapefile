...
export default function() {
  ...
  return that._source.slice(12).then(function(array) {
    ...
    // four-byte increments to find the next valid object. This object is
    ...
    function scan() {
      return that._source.slice(4).then(function(chunk) {
        ...
        header = view(array = concat(array.slice(4), chunk)), type = header.getInt32(8, true);
        return header.getInt32(0, false) !== that._index || (type && type !== that._type) ? scan() : read();
      });
    }

    function read() {
      return that._source.slice(Math.max(0, header.getInt32(4, false) * 2 - 4)).then(function(chunk) {
